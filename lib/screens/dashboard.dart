...
import 'package:nexmove/screens/assigned_trips.dart';
...
class Choice {
  ...
  const Choice(
      {@required this.title, @required this.icon, @required this.imageIcon});
}

class Dashboard extends StatelessWidget {
  final LoginLogoutListener loginLogoutListener;
  final List<Choice> choices = const <Choice>[
    const Choice(title: 'Hoy', icon: Icons.local_shipping, imageIcon: ''),
    const Choice(title: 'Programados', icon: Icons.today, imageIcon: ''),
    const Choice(title: 'Realizados', icon: Icons.done_outline, imageIcon: ''),
    const Choice(
        title: 'Mis Ofertas',
        icon: null,
  ...
  Dashboard(this.loginLogoutListener);

  @override
  Widget build(BuildContext context) {
    int userOfferType =
        Globals.sharedPrefs.getInt(Constants.PREFS_USER_OFFER_TYPE);
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text(
      ...
      drawer: this.drawerMenu(context),
      body: SafeArea(
        child: Container(
